...
const fakeOutputDir = (rejectCreateDir = false, rejectRemoveDir = false) => ({
  createDir: sinon.spy(() => rejectCreateDir ? Promise.reject(new Error()) : Promise.resolve()),
  removeDir: sinon.spy(() => rejectRemoveDir ? Promise.reject(new Error()) : Promise.resolve())
})
