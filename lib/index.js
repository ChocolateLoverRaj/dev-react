...
const babelPluginModuleResolver = (async () => {
  const plugin = (await import('babel-plugin-module-resolver')).default.default
  return babel.createConfigItem(plugin)
})()
  ...
  const createBrowserJs = (page, code) => {
    pages.get(page).browserJsPromise = (async () => {
      const res = await babel.transformAsync(code, {
        ...
        plugins: [[
          await babelPluginModuleResolver,
          {
            alias: {
